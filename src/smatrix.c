...
static int linsolve_timing_enabled = -1;
static int linsolve_timing_registered = 0;
static unsigned long long linsolve_calls = 0;
static unsigned long long linsolve_failures = 0;
static unsigned long long linsolve_total_ns = 0;
static unsigned long long linsolve_factor_ns = 0;
static unsigned long long linsolve_forward_ns = 0;
static unsigned long long linsolve_backward_ns = 0;
static unsigned long long linsolve_setup_ns = 0;
...
static void linsolve_timing_report(void)
{
    ...
    if (!linsolve_timing_enabled)
        return;
    if (linsolve_calls == 0)
        return;

    total_ms = (double)linsolve_total_ns / 1.0e6;
    factor_ms = (double)linsolve_factor_ns / 1.0e6;
    forward_ms = (double)linsolve_forward_ns / 1.0e6;
    backward_ms = (double)linsolve_backward_ns / 1.0e6;
    setup_ms = (double)linsolve_setup_ns / 1.0e6;
    other_ms = total_ms - (factor_ms + forward_ms + backward_ms + setup_ms);
    if (other_ms < 0.0)
        other_ms = 0.0;

    fprintf(stderr,
            "EPANETx linsolve timing: calls=%llu failures=%llu total_ms=%.3f avg_call_ms=%.6f\n",
            linsolve_calls,
            linsolve_failures,
            total_ms,
            total_ms / (double)linsolve_calls);

    fprintf(stderr,
            "EPANETx linsolve phases: factor_ms=%.3f forward_ms=%.3f backward_ms=%.3f setup_ms=%.3f other_ms=%.3f\n",
            factor_ms,
            forward_ms,
            backward_ms,
            setup_ms,
...
static void linsolve_timing_init(void)
{
    const char *env;

    if (linsolve_timing_enabled != -1)
        return;

    env = getenv("EPANETX_LINSOLVE_TIMING");
    linsolve_timing_enabled = (env && env[0] && strcmp(env, "0") != 0) ? 1 : 0;
    if (linsolve_timing_enabled && !linsolve_timing_registered)
    {
        atexit(linsolve_timing_report);
        linsolve_timing_registered = 1;
    }
}
    ...
    for (blink = alink->next; blink != NULL; blink = blink->next)
    {
        ...
        if (jnode > 0 && degree[jnode] > 0) // jnode still active
        {
            if (!linked(net, inode, jnode)) // inode not linked to jnode
            {
                ...
                if (!addlink(net, inode, jnode, sm->Ncoeffs))
                    return 0;
                if (!addlink(net, jnode, inode, sm->Ncoeffs))
                    return 0;
                degree[inode]++;
                degree[jnode]++;
            }
...
*/
{
    ...
    linsolve_timing_init();
    if (linsolve_timing_enabled)
        t_call_start = linsolve_now_ns();

    if (linsolve_timing_enabled)
        t_stage = linsolve_now_ns();
    memset(temp + 1, 0, n * sizeof(double));
    memset(link + 1, 0, n * sizeof(int));
    memset(first + 1, 0, n * sizeof(int));
    if (linsolve_timing_enabled)
    {
        linsolve_setup_ns += linsolve_now_ns() - t_stage;
        t_stage = linsolve_now_ns();
    }
    ...
    for (j = 1; j <= n; j++)
    {
        ...
        while (k != 0)
        {
            ...
            if (istrt < istop)
            {
                ...
                nzp = NZSUB + istrt;
                lnzp = LNZ + istrt;
                nzp_stop = NZSUB + istop;
                for (; nzp < nzp_stop; nzp++, lnzp++)
                {
                    isub = *nzp;
                    lnzi = *lnzp;
                    temp[isub] += Aij[lnzi] * ljk;
                }
            }
            k = newk;
        }
        ...
        if (diagj <= 0.0) // Check for ill-conditioning
        {
            if (linsolve_timing_enabled)
            {
                linsolve_factor_ns += linsolve_now_ns() - t_stage;
                linsolve_total_ns += linsolve_now_ns() - t_call_start;
                linsolve_calls++;
                linsolve_failures++;
            }
            return j;
    ...
    if (linsolve_timing_enabled)
    {
        linsolve_forward_ns += linsolve_now_ns() - t_stage;
        t_stage = linsolve_now_ns();
    }
    ...
    if (linsolve_timing_enabled)
    {
        linsolve_backward_ns += linsolve_now_ns() - t_stage;
        linsolve_total_ns += linsolve_now_ns() - t_call_start;
        linsolve_calls++;
    }
    return 0;
